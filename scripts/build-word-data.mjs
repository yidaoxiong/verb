...
const supplementPath = path.join(outputDir, 'school-supplement.json');
...
function mapSupplementRecord(record) {
  const answerVariants = [...new Set((record.answerVariants || [record.answer || record.word]).map(value => String(value).normalize('NFC')))].filter(Boolean);
  const card = {
    ...record,
    id: `school:${record.sourceIndex}`,
    module: 'school',
    sourceIndex: Number(record.sourceIndex),
    word: String(record.word).normalize('NFC'),
    answer: String(record.answer || record.word).normalize('NFC'),
    answerVariants,
    answerLength: String(record.answer || record.word).length,
    maxAnswerLength: Math.max(...answerVariants.map(value => value.length)),
    source: record.source || { 来源: 'Unit 1 单词句式基础练习(1).pdf' },
  };
  if (record.cardType === 'sentence') {
    card.sentenceParts = (record.sentenceParts || []).map(part => ({
      prompt: String(part.prompt || '').normalize('NFC'),
      answers: [...new Set((part.answers || []).map(value => String(value).normalize('NFC')))].filter(Boolean),
    }));
  }
  return card;
}
...
if (!Array.isArray(supplement) || supplement.length !== 12) throw new Error('school supplement must contain exactly 12 records');
const supplementRows = supplement.map(mapSupplementRecord);
supplementRows.forEach((row, index) => {
  if (row.sourceIndex !== 174 + index || row.id !== `school:${174 + index}`) {
    throw new Error(`school supplement row ${index + 1} has an unstable id or source index`);
  }
  if (row.cardType === 'sentence' && (!row.sentenceParts?.length || row.sentenceParts.some(part => !part.prompt || !part.answers.length))) {
    throw new Error(`${row.id} must contain sentence prompts and answers`);
  }
});
const schoolWithSupplement = [...school, ...supplementRows];
...
if (schoolWithSupplement.length !== 185) throw new Error(`school source plus supplement count ${schoolWithSupplement.length}, expected 185`);
if (houhai.length !== 223) throw new Error(`houhai source count ${houhai.length}, expected 223`);
for (const [module, rows] of [['school', schoolWithSupplement], ['houhai', houhai]]) {
...
await writeFile(path.join(outputDir, 'school-words.js'), dataSource('SchoolWords', schoolWithSupplement), 'utf8');
...
console.log(JSON.stringify({ school: schoolWithSupplement.length, houhai: houhai.length, outputDir }, null, 2));
