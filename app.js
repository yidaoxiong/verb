...
function relatedVerbs(base){const source=verbs.find(verb=>verb.base===base),family=similarFamilies.find(group=>group.includes(base))||[],selected=family.filter(word=>word!==base).map(word=>verbs.find(verb=>verb.base===word)).filter(Boolean);if(selected.length<4){const extras=verbs.filter(verb=>verb.base!==base&&!selected.some(item=>item.base===verb.base)).sort((left,right)=>similarityScore(source,right)-similarityScore(source,left));selected.push(...extras.slice(0,4-selected.length))}return selected.slice(0,4)}
function renderRelated(verb){$('#relatedList').innerHTML=relatedVerbs(verb.base).map(item=>`<article><b>${item.base}</b><span>${item.past} · ${item.participle}</span><small>${meanings[item.base]}</small></article>`).join('')}
